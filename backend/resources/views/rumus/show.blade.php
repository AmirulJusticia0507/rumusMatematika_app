...
@section('content')
    ...
    <section class="mx-auto max-w-4xl px-4 py-12 sm:px-6 lg:px-8">
        @if ($rumusInfo['rumus'] !== null)
            <div class="overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-xl shadow-slate-900/5">
                <div class="flex items-center justify-between border-b border-slate-100 bg-slate-50/70 px-6 py-4">
                    ...
                    <span class="font-mono text-xs font-semibold uppercase tracking-wider text-slate-400">rumus.php</span>
                </div>
                ...
                <div class="flex items-start gap-3 px-6 py-6 sm:px-10">
                    <span class="mt-0.5 flex h-8 w-8 shrink-0 items-center justify-center rounded-lg bg-indigo-100 text-indigo-600">
                    ...
                    <div>
                        <h2 class="text-sm font-bold uppercase tracking-wide text-slate-900">Penjelasan</h2>
                        <p class="mt-1 text-slate-600">{{ $rumusInfo['keterangan'] }}</p>
                    </div>
                </div>
            </div>
        @endif
        ...
        <div class="mt-10 flex flex-col items-center justify-between gap-4 sm:flex-row">
            <a href="{{ route('rumus.index') }}" class="inline-flex w-full items-center justify-center gap-2 rounded-xl bg-white px-6 py-3 text-sm font-bold text-slate-700 shadow-sm ring-1 ring-slate-200 transition hover:bg-slate-50 sm:w-auto">
