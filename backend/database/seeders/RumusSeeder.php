...
class RumusSeeder extends Seeder
{
    public function run(): void
    {
        ...
        $jenjangMap = function (string $title) use ($sd, $smp, $sma): string {
            foreach (['SD' => $sd, 'SMP' => $smp, 'SMA' => $sma] as $level => $list) {
        ...
        $visual = function (string $title) use ($emojis, $gradients): array {
            $hash = 0;
        ...
        foreach ($rumusDetail as $title => $item) {
            $index++;

            [$emoji, $gradient] = $visual($title);
